...
import registerImg from "../../../assets/images/register_aesthetic.png";
...
const Register = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData({
      ...formData,
      [name]: type === "checkbox" || type === "radio" ? (name === "isSeller" ? value === "true" : checked) : value,
    });
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const success = await handleRegisterUser(formData);
    if (success) {
      navigate("/");
    }
  };
      ...
      >
        <div className="max-w-md w-full mx-auto">
          ...
          >
            ...
            <p className="text-gray-400 mb-8 text-lg font-light tracking-wide">Join the SNITCH ecosystem.</p>
          </motion.div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <motion.div
                ...
                className="space-y-2"
              >
                <label className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-500 ml-1">Email</label>
                <div className="relative group">
              ...
              <motion.div
                ...
                className="space-y-2"
              >
                <label className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-500 ml-1">Contact</label>
                <div className="relative group">
                  <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 group-focus-within:text-white transition-colors" />
                  <input
                    type="tel"
                    name="contact"
                    required
            ...
            >
              <label className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-500 ml-1">Account Type</label>
              <div className="flex gap-6">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    ...
                    onChange={handleChange}
                  />
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${formData.isSeller === false ? 'border-white bg-white' : 'border-white/20'}`}>
                    {formData.isSeller === false && <div className="w-2 h-2 rounded-full bg-black" />}
                  </div>
                  <span className={`text-sm tracking-wide transition-colors ${formData.isSeller === false ? 'text-white' : 'text-gray-500'}`}>Customer</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    type="radio"
                    ...
                    onChange={handleChange}
                  />
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${formData.isSeller === true ? 'border-white bg-white' : 'border-white/20'}`}>
                    {formData.isSeller === true && <div className="w-2 h-2 rounded-full bg-black" />}
                  </div>
                  <span className={`text-sm tracking-wide transition-colors ${formData.isSeller === true ? 'text-white' : 'text-gray-500'}`}>Seller</span>
                </label>
              </div>
            </motion.div>
            ...
            <motion.button
              ...
              type="submit"
              className="w-full bg-white text-black font-bold py-5 rounded-none flex items-center justify-center gap-3 hover:bg-gray-200 transition-all mt-8 uppercase tracking-widest text-xs"
            >
              Create Account
            </motion.button>
          ...
          <motion.div
            ...
            className="mt-10 pt-8 border-t border-white/5 flex flex-col items-center"
          >
            <p className="text-gray-500 text-sm">
              Already a member?{" "}
              <Link to="/login" className="text-white hover:text-gray-300 underline underline-offset-4 transition-colors font-medium">
                Access your account
              </Link>
            </p>
          </motion.div>
        ...
        <div className="absolute bottom-16 left-16 max-w-md">
          ...
          >
            ...
            <p className="text-3xl font-light italic text-gray-200 leading-relaxed tracking-tight">
              "Crafting the future of retail, one digital stitch at a time."
            </p>
            <p className="mt-4 text-gray-500 uppercase tracking-widest text-[10px] font-bold">Snitch / Universal Registry</p>
          </motion.div>
        </div>
      </motion.div>
    </div>
